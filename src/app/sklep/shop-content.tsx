  ...
  return (
    <div>
      ...
      <header className="relative overflow-hidden border-b border-ink-300 bg-ink-50">
        <Splatter className="right-[-30px] top-[-40px] w-[260px]" opacity={0.26} rotate={120} />
        <div className="relative mx-auto max-w-[1340px] px-4 py-14 lg:px-8 lg:py-16">
          <div className="mb-2 font-mono text-[11px] uppercase tracking-[0.3em] text-acid">// sklep</div>
          ...
          <div className="flex flex-wrap gap-2">
            {shopFilters.map((f) => (
              <button
                ...
                className={cn(
                  'text-graffiti px-3 py-1.5 text-xs tracking-[0.12em] transition-all',
                  filter === f.key
                    ? 'bg-acid text-bone shadow-[2px_2px_0_#060706]'
                    : 'border border-ink-300 text-bone/70 hover:border-acid hover:text-acid',
                )}
              >
                {f.label}
              </button>
